...
namespace AspNetCoreRateLimit
{
    public class UserRateimiterMiddleWare
    {
        ...
        private readonly UserRateLimitProcessor _processor;
        ...
        {
            var message = string.IsNullOrEmpty(_options.QuotaExceededMessage) ? $"API calls quota exceeded for User {httpContext.User.Identity.Name}! maximum admitted {rule.Limit} per {rule.Period}." : _options.QuotaExceededMessage;
        ...
        {
            _logger.LogInformation($"Request {identity.HttpVerb}:{identity.Path} from User {identity.User} has been blocked, quota {rule.Limit}/{rule.Period} exceeded by {counter.TotalRequests}. Blocked by rule {rule.Endpoint}, TraceIdentifier {httpContext.TraceIdentifier}.");
        }
